...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs != 7) {
        mexErrMsgIdAndTxt(
                "fCWT:nrhs", "Six inputs required. (inputArray, c0, fs, f0, f1, fn, nthreads)");
    }
    if (nlhs != 2) {
        mexErrMsgIdAndTxt(
                "fCWT:nlhs", "Two outputs are required. (time-frequency matrix, frequencies)");
    }
    ...
    if (mxGetM(prhs[0]) != 1) {
        mexErrMsgIdAndTxt("fCWT:notRowVector", "Input must be a row vector.");
    }

    if (!mxIsDouble(prhs[1]) || mxIsComplex(prhs[1]) || mxGetNumberOfElements(prhs[1]) != 1) {
        mexErrMsgIdAndTxt("fCWT:notScalar", "Wavelet coefficient must be scalar.");
    }

    if (!mxIsDouble(prhs[2]) || mxIsComplex(prhs[2]) || mxGetNumberOfElements(prhs[2]) != 1) {
        mexErrMsgIdAndTxt("fCWT:notScalar", "Sampling frequency must be a scalar.");
    }
    if (!mxIsDouble(prhs[3]) || mxIsComplex(prhs[3]) || mxGetNumberOfElements(prhs[3]) != 1) {
        mexErrMsgIdAndTxt("fCWT:notScalar", "Starting frequency must be a scalar.");
    }
    if (!mxIsDouble(prhs[4]) || mxIsComplex(prhs[4]) || mxGetNumberOfElements(prhs[4]) != 1) {
        mexErrMsgIdAndTxt("fCWT:notScalar", "Ending frequency must be a scalar.");
    }
    if (!mxIsDouble(prhs[5]) || mxIsComplex(prhs[5]) || mxGetNumberOfElements(prhs[5]) != 1) {
        mexErrMsgIdAndTxt("fCWT:notScalar", "Number of frequencies must be a scalar.");
    }
    if (!mxIsDouble(prhs[6]) || mxIsComplex(prhs[6]) || mxGetNumberOfElements(prhs[6]) != 1) {
        mexErrMsgIdAndTxt("fCWT:notScalar", "Number of threads must be a scalar.");
    }
