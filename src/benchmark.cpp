...
static void show_usage(std::string name) {
    std::cerr << "Usage: ./fcwt <method> <number of samples> <number of threads> "
                 "<OPTIONAL:optimization method>\n"
              << "Method:\n"
              << "\t-h,--help\t Show this help message\n"
              << "\t-fcwt\t\t Run fCWT benchmark for N samples and K threads\n"
              << "\t-fcwtoptimize\t Calculate fCWT optimization plan for N samples, K threads "
...
int main(int argc, char *argv[]) {
    ...
    if (regex_match(argv[1], letterregex) && regex_match(argv[2], numberregex) &&
        regex_match(argv[3], numberregex)) {
        algorithm = argv[1];
        size = stoi(argv[2]);
        ...
        cerr << "ERROR: Please define the method, number of samples and number of threads "
                "correctly."
             << '\n';
        return 0;
    }
    ...
    if (algorithm == "-fcwtoptimize") {
        if (argc == 5) {
            if (regex_match(argv[4], letterregex)) {
                ...
                cout << "ERROR: No optimization method recognized, use -h to see which methods are "
                        "available"
                     << '\n';
                return 0;
            }
            ...
            optimization = "measure";
            cout << "ERROR: No optimization method defined, use -h to see which methods are "
                    "available"
                 << '\n';
            return 0;
        }
    ...
    chrono::duration<double> *times =
            (chrono::duration<double> *)malloc(sizeof(chrono::duration<double>) * runs);
