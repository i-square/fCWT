...
void FCWT::daughter_wavelet_multiplication(
        fftwf_complex *input,
...
void FCWT::create_FFT_optimization_plan(int maxsize, int flags) {
    ...
    for (int i = 11; i <= nt; i++) {
        ...
        std::cout << "Optimization schemes for N: " << n
                  << " have been calculated. Next time you use fCWT it will automatically choose "
                     "the right optimization scheme based on number of threads and signal length."
                  << std::endl;
    }
}
...
void FCWT::load_FFT_optimization_plan() {
    ...
    if (use_optimalization_schemes) {
        ...
        if (!fftwf_import_wisdom_from_filename(file_for)) {
            std::cout << "WARNING: Optimization scheme '" << file_for
...
void FCWT::fftbased(
    ...
    // Perform daughter wavelet generation and multiplication with the Fourier transformed input
    // signal
